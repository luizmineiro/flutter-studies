...
class SharedPreferencesToBuyLocalDataRepository
  ...
  Future<List<HiveToBuyModel>> loadToBuyList() async {
    try {
      ...
      if (toBuyListJSON != null) {
        ...
        //* transformar o List<Map<String, dynamic>> para List<HiveToBuyModel>
        final toBuyList = (toBuyDecoded as List)
            .map((toBuy) => HiveToBuyModel.fromMap(toBuy))
            .toList();
  ...
  Future<void> addToBuyItem(HiveToBuyModel buyModel) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      //* carregar a lista
      final List<HiveToBuyModel> toBuyList = await loadToBuyList();
